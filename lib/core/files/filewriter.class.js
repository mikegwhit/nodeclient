...
class FileWriter {
    ...
    static createFile(fromFolder, filename, contents, packageName, temporary) {
        ...
        if (packageName) {
            ...
            if (!require('fs').existsSync(path)) {
                console.error('Tried to create file under package that DNE', 
                    packageName, path);
                return;
            }
        }
    ...
    /**
     * Segments large files into specified chunkSize.  This is handy for 
     * replication.
     * @param {String} src The source file.
     * @param {String} dest Tge dest filename.  This gets .partX as a suffix
     *  where X is the part number.
     * @param {Number} chunkSize The number of bytes to divide by.  Default
     * is 2MB.
     */
    createFileSegments(src, dest, chunkSize = 2000000, suffix = '.part') {
        const size = require('fs').statSync(src).size;
        const chunks = size / chunkSize;
        if (size < chunkSize) {
            return 1;
        }
        const fd = require('fs').openSync(src, 'r');
        const destFolder = dest.split('/').slice(0, -1).join('/');
        if (!require('fs').existsSync(destFolder)) {
            require('fs').mkdirSync(destFolder);
        }
        for (let chunk = 0; chunk < chunks; chunk++) {
            const buffer = Buffer.alloc(Math.min(size - chunk * chunkSize, 
                chunkSize));
            const bytes = require('fs').readSync(fd, buffer, 0, 
                Math.min(size - chunk * chunkSize, chunkSize), 
                chunk * chunkSize);
            require('fs').writeFileSync(`${dest}${suffix}${chunk}`, 
     ...
     * @param  {boolean} temporary  If temporary, will be deleted on shutdown.
     */
    static createPackageFile(fromFolder, filename, contents, packageName, 
        temporary) {
            FileWriter.createFile(fromFolder, filename, contents, packageName, 
                temporary);
        }
     ...
     * @param {String} src The base filename to join.  Reads all files 
     * matching with a .partX suffix.
     * @param {String} dest Where to dump the joined file.
     * @param {Number} numParts (Optional) If provided, verifies the number
     * of parts used to join the file is correct.  If numParts is set to -1,
     * then checks until no more file parts are found.
     * @param {String} suffix The suffix to search for with.
     */
    joinFileParts(src, dest, numParts = -1, suffix = '.part') {
        const srcFolder = src.split('/').slice(0, -1).join('/');
        const filename = src.split('/').pop();
        let part = 0;
        let pos = 0;
        const fd = require('fs').openSync(dest, 'w');
        while (require('fs').existsSync(`${src}${suffix}${part}`)) {
            const buffer = require('fs').readFileSync(`${src}${suffix}${part}`);
            require('fs').writeSync(fd, buffer, 0, null, pos);
            pos += buffer.toString('utf8').length;
            part++;
        }
        require('fs').closeSync(fd);
    }
    ...
    static writeFile(filepath, contents, overwrite = true, encoding = 'utf8',
        ...
        if (temporary && !FileWriter.temporaryFiles.includes(FileHelpers
            .normalize(process.cwd() + '/' + filename))) {
                // Store this so we delete it at exit.
                FileWriter.temporaryFiles.push(FileHelpers.normalize(process
                    .cwd() + '/' + filename));
            } else if (!temporary) {
                try {
                    FileWriter.temporaryFiles
                        .splice(FileWriter.temporaryFiles
                        .indexOf(FileHelpers.normalize(process.cwd() + '/' + 
                        filename)), 1);
                } catch(e) {
                }
            }
